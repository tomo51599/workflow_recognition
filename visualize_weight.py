...
while cap.isOpened():
    ...
    if frame_No > 49 and frame_No % 48 == 0:
        ...
        # 確率に重みを適用
        adjusted_probabilities = apply_weights_to_prediction(predict_probability[0], weight)
        
        # 最終推論を決定
        adjusted_predictions = determine_adjusted_prediction(adjusted_probabilities, class_names)
        
        predicted_list.append(adjusted_predictions)
        
        predict_count = [class_name_to_index[p] + 1 for p in adjusted_predictions]
        ...
        if len(predicted_list) > 9:
            ...
            if determined_phase != dominant_phase and not None and dominant_phase is not None:
                ...
                weight += 1
                print(weight)
                frame_phase_changed.append(frame_No - 240)
        ...
        rounded_adjusted_probability = np.around(np.array(adjusted_probabilities), decimals = 3)
                                
        result_dict[frame_No] = {
                'predicted_class': adjusted_predictions,  # 最初の予測ラベルを格納
                'actual_class': f"phase{current_phase}",
                'dominant_class':dominant_phase
            }
    ...
    if predicted_labels is not None:
        ...
        cv2.putText(frame_2,
                        f"adjusted_score:{rounded_adjusted_probability} ",
                        (10, 105),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.7,
                        (0, 0, 0),
        ...
        cv2.putText(frame_2,
                f"adjusted_predict:{adjusted_predictions} dominant_phase:['{dominant_phase}'] times_phase_changed:{weight}",
                (10, 130),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (0, 0, 0),
...
plt.savefig(f'/home/master_thesis/results/confusion_matrix_{xml_no}.png')    
